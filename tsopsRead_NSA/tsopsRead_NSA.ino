...
void parseData() {
  ...
  strtokIndx = strtok(NULL, ","); // this continues where the previous call left off - get NSA position
  newNsaPos = atoi(strtokIndx);          // convert this part to integer 
  
  strtokIndx = strtok(NULL, ","); // this continues where the previous call left off - get NOC position
...
void updateLCD() {
  ...
  if (newNsaPos != NsaPos) {
      // print new NSA position to screen
    lcd.setCursor(8,0);
    lcd.print("  "); // clear previous entry
    lcd.setCursor(8,0);
    lcd.print(newNsaPos);
