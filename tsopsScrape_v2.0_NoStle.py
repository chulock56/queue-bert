...
def scrapeTsops():
    ...
    try:
        queuePosTableEl = driver.find_element_by_id('positions-container')
    ...
    if len(queuePosTableVals) != 0:
        ...
        queuePosTableStr = '99,99,99'

    # Scrape staleness value
    # try:
        # agentTableEl = driver.find_element_by_id('big-table')
        # agentTableText = agentTableEl.text

        # myAgentRowMatch = re.search(r'Andrew Chulock\n([1-7]?\.?\d\d:\d\d:\d\d|> 1 week)',
                                    # agentTableText)  # find either 9.99:99:99, 99:99:99, or > 1 week
    # except:
    ...
    tsopsData = '<' + queuePosTableStr + '>'  # append start and end markers
    # print(tsopsData)
...
path = driver.install(browser=driver.chrome, file_directory=driverDir, verbose=True, chmod=True, overwrite=False,
                      version=None)
...
options.add_argument('--log-level=1')  # used to suppress extraneous messages. Defualt value is 0 (INFO)
...
try:
    while True:
        ...
        time.sleep((
                               runInt + offset) - time.time() % runInt)  # will run loop every X seconds + offset, locked to system clock (i.e. 00:00:06, 00:00:11, etc.)
